...
- select the SAM neighborhood radius through a validation sweep when time permits;
...
## Optional SAM rho Sweep

Before the final SAM run, use a validation split from the CIFAR-10 training set to choose the SAM neighborhood radius:

```bash
python run_rho_sweep.py
```

The sweep only applies to SAM because `rho` controls SAM's local perturbation radius. SGD does not have this hyperparameter. The configured sweep values are:
...
Use the best validation result to set `SAM_RHO` in `config.py`, then rerun `python run_experiment.py` for the final SGD vs SAM comparison.
...
`lr_schedule_curve.png` requires training logs with an `lr` column. `rho_sweep.png` requires `results/rho_sweep.csv`, which is generated by `run_rho_sweep.py`.
